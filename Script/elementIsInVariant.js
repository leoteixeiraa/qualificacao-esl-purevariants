...
function exportModels(models) {
	...
	var fw = new java.io.FileWriter(new java.io.File(pv_module.getOutput(), filename));
	//create headline
...
function writeElementLines(fw, element) {
	var row = new Array();
	var vdm = pv_module.getVariantModel();
	...
	while (iter.hasNext()) {
		...
		row[0] = e.getName();
		if (vdm.getSelection(e) == VariantElementState().SELECTION) {
			row[1] = "true";
		} else {
			row[1] = "false";
		}
